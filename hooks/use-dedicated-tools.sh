...
if [[ "$command_str" =~ \$\( ]]; then
  ...
  if [[ "$command_str" =~ ^git\ commit ]]; then
    deny_tool "Command substitution (\$(...)) is not allowed. Read tmp/commit-msg.txt first (if it exists), then write the commit message there using the Write tool (title ≤50 chars, body wrapped at 72 cols, blank line between title and body), then run: git commit -F tmp/commit-msg.txt — if tmp/ does not exist, run 'mkdir -p tmp' and ensure tmp/ is in .gitignore."
  else
